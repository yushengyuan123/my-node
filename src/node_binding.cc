...
// 这个void _register_##modname函数也是一个宏，定义在同名的头文件下
//他本质上是一个node_module_register函数
...
// 入参(env, mod, module) env是node的环境对象
...
// args = fs
// 可以看到这里有一些v8的东西在里面，因为这个函数是被js调用的
// 去寻找一些c++模块
void GetInternalBinding(const FunctionCallbackInfo<Value>& args) {
  // 通过v8上下文拿到这个node的Environment对象
  // 这个对象是被绑定在了v8 Context中的
  Environment* env = Environment::GetCurrent(args);
  ...
  Local<Object> exports;
  // modlist_internal这个就是我们的模块链表，在register的时候会把c++模块插入到
  // 这个链表中。返回找到的结构体node_module，找到的模块
  node_module* mod = FindModule(modlist_internal, *module_v, NM_F_INTERNAL);
  if (mod != nullptr) {
    // mod不是空指针，说明找到了模块，这个时候就初始化这个模块
