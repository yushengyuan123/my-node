...
ChildProcess.prototype.spawn = function(options) {
  ...
  // return { stdio, ipc, ipcFd };这个函数选择了父子进程之间通过什么方式进行的通信
  // 初始化了一个Pipe类
...
// stdio, false,let stdio = options.stdio || 'pipe';
function getValidStdio(stdio, sync) {
  ...
  if (typeof stdio === 'string') {
    // 里面把stdio转成了一个数组，里面感觉使用了一个c++的数组方法但是我找不到
    // [stdio, stdio, stdio]
  ...
  stdio = ArrayPrototypeReduce(stdio, (acc, stdio, i) => {
    ...
    // 这里就是根据stdio的类型去决定new Pipe传入的参数是什么
    // 如果是pipe或者overlapped那么参数都是PipeConstants.SOCKET
    // ipc的话那么就是PipeConstants.ipc
    if (stdio === 'ignore') {
